...
export function AnalysisPage() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    if (!documentId || !opportunityId) {
      add('error', t('Please select both a document and an opportunity'))
      return
    }
    ...
    try {
      ...
      setResult(res.data)
      add('success', t('Analysis complete'))
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <h2 className="mt-1 text-lg font-semibold text-[var(--color-text)]">{t("Resume Analysis")}</h2>
      ...
      <form onSubmit={handleAnalyze} className="border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="label">{t("Resume / Document")}</label>
            <select
              className="input"
              value={documentId}
            ...
            {documents.length === 0 && (
              <p className="mt-1 text-xs text-[var(--color-text-placeholder)]">{t("No documents available. Upload one first.")}</p>
            )}
          </div>
          <div>
            <label className="label">{t("Opportunity")}</label>
            <select
              className="input"
              value={opportunityId}
              ...
              required
            >
              <option value="">{t("Select an opportunity...")}</option>
            ...
            {opportunities.length === 0 && (
              <p className="mt-1 text-xs text-[var(--color-text-placeholder)]">{t("No opportunities available. Add one first.")}</p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-[var(--color-text-placeholder)]">{t("Match Score")}</p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
              ...
              {result.matched.length > 0 ? (
                ...
                <p className="text-sm text-[var(--color-text-placeholder)]">{t("No matched skills")}</p>
              )}
            </div>
            <div className="border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
              <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-[var(--color-text)]">
              ...
              {result.missing.length > 0 ? (
                ...
                <p className="text-sm text-[var(--color-text-placeholder)]">{t("No missing skills")}</p>
              )}
            </div>
          </div>
          ...
          {result.parse_ability_score != null && (
            <div className="flex items-center justify-between border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3">
              <div>
                <p className="text-sm font-medium text-[var(--color-text)]">{t("ATS / Parseability")}</p>
                <p className="text-xs text-[var(--color-text-placeholder)]">{t("How readable your document is for automated systems")}</p>
              </div>
          ...
          {result.report_text && (
            <div className="border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
              <h3 className="mb-2 text-sm font-semibold text-[var(--color-text)]">{t("Recommendations")}</h3>
      ...
      {!result && (
        <div className="border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-12 text-center">
          ...
          <p className="mt-2 text-sm font-medium text-[var(--color-text-secondary)]">{t("Ready to analyze")}</p>
          <p className="mt-0.5 text-xs text-[var(--color-text-placeholder)]">
            {t("Select a document and opportunity above, then click Analyze Resume.")}
          </p>
        </div>
      )}
    </div>

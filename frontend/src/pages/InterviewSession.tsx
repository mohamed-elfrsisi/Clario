...
export function InterviewSessionPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!result.accepted) {
        setError(result.message ?? t('Answer could not be submitted.'))
        return
      }
      setSubmitted(true)
      if (result.completed) {
      ...
      setError(t('We could not submit this answer. Please try again.'))
    } finally {
      setSubmitting(false)
    }
  }

  if (loading) return <div className="mx-auto flex min-h-[60vh] max-w-3xl items-center justify-center"><Card className="w-full p-8 text-center"><div className="mx-auto h-7 w-7 animate-spin rounded-full border-2 border-[var(--color-accent)] border-t-transparent" /><p className="mt-3 text-sm text-[var(--color-text-secondary)]">{t("Loading your interview...")}</p></Card></div>
  if (error && !question) return <div className="mx-auto max-w-2xl"><Card className="p-8 text-center"><AlertCircle className="mx-auto h-7 w-7 text-[var(--color-error-text)]" /><h1 className="mt-4 text-xl font-semibold">{t("Interview unavailable")}</h1><p className="mt-2 text-sm text-[var(--color-text-secondary)]">{error}</p><Button className="mt-6" onClick={() => navigate('/mock-interviews')}>{t("Back to interviews")}</Button></Card></div>
      ...
      <Card className="my-8 flex flex-1 flex-col p-6 sm:p-10">
        <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-accent-text)]">{t("Interview question")}</p><h1 className="mt-4 text-2xl font-semibold leading-tight tracking-tight sm:text-3xl">{question.prompt}</h1><p className="mt-4 text-sm leading-6 text-[var(--color-text-secondary)]">{t("Answer as you would in a real interview. Use specific evidence from your experience where relevant.")}</p>
          <div className="mt-8 flex flex-1 flex-col"><label htmlFor="interview-answer" className="text-sm font-medium">{t("Your answer")}</label><textarea id="interview-answer" autoFocus value={answer} onChange={(event) => setAnswer(event.target.value)} disabled={submitting || submitted} placeholder={t("Write your answer here...")} className="mt-2 min-h-[240px] flex-1 resize-none rounded-[var(--radius-lg)] border border-[var(--color-input-border)] bg-[var(--color-input-bg)] p-4 text-sm leading-7 text-[var(--color-text)] placeholder:text-[var(--color-text-placeholder)] focus:border-[var(--color-focus)] focus:outline-none focus:ring-2 focus:ring-[var(--color-focus-ring)] sm:min-h-[300px]" />
            ...
            {submitted && <p className="mt-3 text-sm font-medium text-[var(--color-success-text)]">{t("Answer submitted. Loading the next question...")}</p>}
          </div>
          <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-[var(--color-text-muted)]">{t("Your next question may change based on the interview service response.")}</p><Button size="lg" onClick={submit} loading={submitting} disabled={submitted}><Send className="h-4 w-4" /> {t("Submit Answer")}</Button></div>
        </div>
      </Card>
    </div>
  )
